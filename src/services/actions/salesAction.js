...
export const getSectionAddr = (section) => {
  ...
  if (section === 'Other Buyer') {
    return newSection.substring(0, 5)
      .concat('_SALE');
  } else if (section === 'Other Purchase') {
    return newSection.substring(0, 5)
      .concat('_BUY');
  } else if (section === 'Purity') return 'OTHER_PURITY';
  if (index < 0) return newSection;
  return newSection.substring(0, index)
    .concat('_', newSection.substring(index + 1));
};
